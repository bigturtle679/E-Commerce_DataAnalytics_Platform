...
export default function AnalyticsPage() {
  ...
  const totalRevenue = useMemo(() =>
    revenueQ.data?.reduce((s, r) => s + r.total_revenue, 0) ?? 0, [revenueQ.data]);
  const totalOrders = useMemo(() =>
    revenueQ.data?.reduce((s, r) => s + r.order_count, 0) ?? 0, [revenueQ.data]);
  const latestCustomers = useMemo(() =>
    customersQ.data?.[0]?.total_customers ?? 0, [customersQ.data]);
  const avgOrderValue = useMemo(() =>
    totalOrders > 0 ? totalRevenue / totalOrders : 0, [totalRevenue, totalOrders]);
        ...
        <div className="max-w-[1600px] mx-auto space-y-3 sm:space-y-4 lg:space-y-5">
            ...
            >
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={orderData}>
                  ...
                  />
                  <Tooltip contentStyle={tooltipStyle} formatter={(value, name) => [formatNumber(Number(value)), name === "order_count" ? "Orders" : "Items"]} />
              ...
              <ResponsiveContainer width="100%" height={240}>
                <AreaChart data={customerData}>
                  ...
                  <YAxis
                    ...
                    tickFormatter={formatNumber}
                  />
                  <Tooltip contentStyle={tooltipStyle} formatter={(value, name) => [formatNumber(Number(value)), name === "total_customers" ? "Total" : "New"]} />
            ...
            >
              <ResponsiveContainer width="100%" height={240}>
                <PieChart>
                  ...
                  <Tooltip contentStyle={tooltipStyle} formatter={(value) => [formatNumber(Number(value)), "Customers"]} />
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="visible">
            <ChartContainer
              ...
              stateStatus="healthy"
            >
              <div className="max-h-[280px] overflow-y-auto overflow-x-auto thin-scrollbar">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="text-xs w-8 sticky top-0 bg-card/80 backdrop-blur-sm">#</TableHead>
                      <TableHead className="text-xs sticky top-0 bg-card/80 backdrop-blur-sm">Product</TableHead>
                      <TableHead className="text-xs sticky top-0 bg-card/80 backdrop-blur-sm">Category</TableHead>
                      <TableHead className="text-xs text-right sticky top-0 bg-card/80 backdrop-blur-sm">Revenue</TableHead>
                      <TableHead className="text-xs text-right sticky top-0 bg-card/80 backdrop-blur-sm">Units</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {productsQ.data?.map((p, i) => (
                      <TableRow key={p.product_id} className="hover:bg-white/[0.03] transition-colors">
                        <TableCell className="text-xs text-muted-foreground py-2">{i + 1}</TableCell>
                        <TableCell className="text-xs font-medium py-2 max-w-[200px] truncate tracking-wide">
                          {p.product_name ?? p.product_id}
                        </TableCell>
                        <TableCell className="text-xs text-muted-foreground py-2">{p.category ?? "—"}</TableCell>
                        <TableCell className="text-xs text-right py-2 font-medium font-mono">{formatCurrency(p.total_revenue)}</TableCell>
                        <TableCell className="text-xs text-right py-2 font-mono">{formatNumber(p.units_sold)}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
