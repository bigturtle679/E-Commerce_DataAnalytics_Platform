...
export default function PipelinePage() {
  ...
  const totalFailures = useMemo(() =>
    statsQ.data?.reduce((s, t) => s + t.failures, 0) ?? 0, [statsQ.data]);
  const totalRowsProcessed = useMemo(() =>
    statsQ.data?.reduce((s, t) => s + t.total_rows_processed, 0) ?? 0, [statsQ.data]);
  ...
  const throughputData = useMemo(() =>
    [...(timelineQ.data ?? [])].reverse().slice(-24), [timelineQ.data]);
        ...
        <div className="max-w-[1600px] mx-auto space-y-3 sm:space-y-4 lg:space-y-5">
          ...
          <div className="grid grid-cols-1 gap-3 sm:gap-4 lg:grid-cols-4">
            <motion.div
              ...
              initial="hidden"
              animate="visible"
              className="hud-panel flex flex-col items-center justify-center rounded-xl p-5 relative overflow-hidden"
            >
            ...
            <StaggerContainer className="col-span-1 grid grid-cols-1 gap-3 sm:gap-4 sm:grid-cols-3 lg:col-span-3">
              <StaggerItem>
                <MetricCard
              ...
              >
                <ResponsiveContainer width="100%" height={220}>
                  <AreaChart data={throughputData}>
                    ...
                    />
                    <Tooltip contentStyle={tooltipStyle} formatter={(value) => [formatNumber(Number(value)), "Rows"]} />
                    <Area
                      type="monotone"
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="visible">
            <Tabs defaultValue="runs">
              ...
              <TabsContent value="runs" className="mt-3">
                ...
                >
                  <div className="max-h-[300px] overflow-y-auto overflow-x-auto thin-scrollbar">
                    <Table>
                      <TableHeader>
                        <TableRow>
                          <TableHead className="text-xs sticky top-0 bg-card/80 backdrop-blur-sm">Pipeline</TableHead>
                          <TableHead className="text-xs sticky top-0 bg-card/80 backdrop-blur-sm">Task</TableHead>
                          <TableHead className="text-xs sticky top-0 bg-card/80 backdrop-blur-sm">Status</TableHead>
                          <TableHead className="text-xs text-right sticky top-0 bg-card/80 backdrop-blur-sm">Rows</TableHead>
                          <TableHead className="text-xs text-right sticky top-0 bg-card/80 backdrop-blur-sm">Duration</TableHead>
                          <TableHead className="text-xs text-right sticky top-0 bg-card/80 backdrop-blur-sm">Started</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {runsQ.data?.map((run) => (
                          <TableRow key={run.id} className="hover:bg-white/[0.03] transition-colors">
                ...
                >
                  <div className="overflow-x-auto thin-scrollbar">
                    <Table>
                      ...
                      <TableBody>
                        {statsQ.data?.map((s) => (
                          <TableRow key={s.task_name} className="hover:bg-white/[0.03] transition-colors">
